...
### Alpha diversity is not significant between vegan and omnivore diets
...
### Diet types are not clustered by either Bray-Curtis or Jaccard dissimilarity
To determine the beta diversity of each sample, PCoA plots using the Bray-Curtis and Jaccard distances were assessed. The Bray-Curtis method is quantitative and uses species abundance while the Jaccard method measures for the absence or presence of species. For the Bray-Curtis plot, each PCoA explain variance of 32% and 21.5%, for a total of 53.5% of variation (Figure 4A). There was no discernable clustering observed for each diet type with the PERMANOVA p value = 0.162 which is greater than 0.05 supporting that there is no statistically significant difference between diet types. For the Jaccard plot, each PCoA explain 15.7% and 13.9% for a total of 29.6% of variation. Similar to the Bray-Curtis PCoA plot, no distinct clustering can be observed by diet type (Figure 4B). PERMANOVA p value = 0.168 indicating no statistically significant differences between diet types.
